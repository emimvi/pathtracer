...
impl Glossy {
    ...
    pub fn f(&self, wo: Vec3, wi: Vec3, normal: Vec3) -> Vec3 {
        ...
        self.reflectance_color * d * g * fresnel / (4. * cos_theta_i * cos_theta_o)
    }
}
...
impl MicrofacetDistribution for GGX {
    ...
    fn D(&self, half_vector: Vec3, macro_normal: Vec3) -> f64 {
        ...
        alpha_g_sqr / (f64::consts::PI * f64::powi(cos_theta, 4) * (alpha_tan2 * alpha_tan2))
    }
...
impl FresnelDielectric {
    ...
    pub fn dielectric(&self, cos_theta_i: f64) -> f64 {
        ...
        (fresnel_par * fresnel_par + fresnel_perp * fresnel_perp) * 0.5
    }
}
...
fn test_fresnel() {
    ...
    assert!(
        f64::abs(r - exp) < 0.0001,
        "Fresnel error: Got {}, expected {}",
        r,
        exp
    );
}
